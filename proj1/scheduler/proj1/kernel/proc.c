 ...
 */
  ...
  int sum, total, lottery_winner;
  struct proc *ticket_counter;
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
    ...
    if (q == 15) {
        ...
        ticket_counter = rp;
        total = sum = lottery_winner = 0;

        /* count the total number of tickets in the queue */
        while (ticket_counter != NULL) {
            total += ticket_counter->ntickets;
            ticket_counter = ticket_counter->p_nextready;
        }

        /* choose a lotto winner */
        lottery_winner = (rand()%total-1) + 1;



        /* find the process who holds the lotto ticket */
        while (rp != NULL) {
            sum += rp->ntickets;
       /*     printf("LOTTERY: %d\t TOTAL: %d\t SUM: %d\t NTICKETS: %d\n",lottery_winner, total, sum, rp->ntickets); */
            if (lottery_winner - sum <= 0) {
                break;
            }
            else
                rp = rp->p_nextready;
